...
# Create pT stages category (> pT1)
Data_full <- Data_full %>%
  mutate(
    stage_high = factor(
      ifelse(as.numeric(stage) > 2, "High pT", "Low pT")
    )
  )
...
# Patient's race
status <- Data$race
gsurv(FU, outcome, status) + ggtitle("Patient's race and overall mortality")
# size_group
status <- Data$size_group
gsurv(FU, outcome, status) + ggtitle("Tumor size and overall mortality")
# stage_high
status <- Data$stage_high
gsurv(FU, outcome, status) + ggtitle("High tumor stage (>pT1) and overall mortality")
# fuhrman_high
status <- Data$fuhrman_high
gsurv(FU, outcome, status) + ggtitle("High Fuhrman grade (> Grade 2) and overall mortality")
# multifocal
status <- Data$multifocal
gsurv(FU, outcome, status) + ggtitle("Tumor multifocality and overall mortality")
# free_margin
status <- Data$free_margin
gsurv(FU, outcome, status) + ggtitle("Tumor invasion of surgical margins and overall mortality")
# progression
status <- Data$progression
gsurv(FU, outcome, status) + ggtitle("Tumor progression and overall mortality")
```
